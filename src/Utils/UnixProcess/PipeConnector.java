 ...
 * the next process
 * <p>
 * This class is necessary to implement pipelines because ProcessBuilder only
 * allows redirection to Files, not to InputStreams or OutputStreams, and
 * Process returns its I/O streams as InputStreams and OutputStreams, which
 * cannot be connected, rather than PipedInputStreams and PipedOutputStreams.
 *
 * <p>
 * Start with:<br>
 * Thread thread = new Thread (new PipeManager (input, output), "thread
 * name");<br>
 * thread.start();
 *
...
public class PipeConnector implements Runnable {
     ...
     *
     * @param process1Output The output stream from the first process (read as
     * an InputStream)
     * @param process2Input The input stream to the second process (written as
     * an OutputStream)
     */
